...
function ConfidencePill({ level }: { level: Confidence }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded border text-[11px] font-mono font-medium ${map[level]}`}>
...
function LifecycleBadge({ status }: { status: LifecycleStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded border text-[11px] font-mono font-medium ${map[status]}`}>
...
function RoHSBadge({ status }: { status: RoHSStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[11px] font-mono font-medium ${map[status]}`}>
      {status === "Compliant"
...
function DiscrepancyTypeBadge({ type }: { type: DiscrepancyType }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border text-[11px] font-medium ${map[type]}`}>
      {icon[type]}{type}
    </span>
  );
}
...
function UploadCenter({ onExtract }: { onExtract: () => void }) {
  ...
  return (
    <div className="space-y-7">
      <div>
        <h2 className="text-xl font-bold text-foreground mb-1" style={{ fontFamily: "Archivo, sans-serif" }}>
          Multi-Document Upload Center
        </h2>
        <p className="text-sm text-muted-foreground">
          Upload all three source documents to begin the AI extraction pipeline.
        </p>
      </div>
            ...
            <tbody>
              {parts.map((p, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-primary">{p.partNumber}</td>
      ...
      <div className="relative bg-card border border-border rounded-2xl shadow-2xl w-full max-w-xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-foreground" style={{ fontFamily: "Archivo, sans-serif" }}>
...
function Header({
  isDark, toggleDark, onNomenclature, active,
}: {
  isDark: boolean;
  toggleDark: () => void;
  onNomenclature: () => void;
  ...
  return (
    <header className="h-13 min-h-[52px] flex items-center px-5 gap-6 border-b border-border bg-card/80 backdrop-blur-sm flex-shrink-0">
      {/* Logo */}
      <div className="flex items-center gap-2.5 flex-shrink-0">
        ...
        <div>
          <div className="text-sm font-bold text-foreground leading-none" style={{ fontFamily: "Archivo, sans-serif" }}>
            BOM Validator
          </div>
      ...
      <div className="flex-1 flex justify-center">
        <PipelineStepper active={active} />
      </div>

      {/* Actions */}
      <div className="flex items-center gap-1.5 flex-shrink-0">
        <button
          onClick={onNomenclature}
        ...
        >
          <BookOpen className="w-3.5 h-3.5" /> Nomenclature
        </button>
        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:opacity-90 transition-all shadow-sm">
          <Plus className="w-3.5 h-3.5" /> New Extraction
        </button>
        <button
          onClick={toggleDark}
          className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-all"
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar active={activeView} onNav={setActiveView} badge={unresolvedCount} />
